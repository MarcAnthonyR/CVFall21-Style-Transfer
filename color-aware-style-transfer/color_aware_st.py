...
import cams_loss, content_loss, style_loss # Added imports for the tests. 
...
STYLE_IMAGE = "color-aware-style-transfer/nst_images/8-style.jpg"
CONTENT_IMAGE = "color-aware-style-transfer/nst_images/1-content.jpg"
...
save_image(output.detach().squeeze(0), 'output.jpg')

# Create and run tests to evaluate nst and cams
output = Image.open('output.jpg')
targetS = Image.open(STYLE_IMAGE)
targetC = Image.open(CONTENT_IMAGE)

cams_loss_result = cams_loss.cams_loss(output, targetS)
content_loss_result = content_loss.content_loss(output, targetC)
style_loss_result = style_loss.style_loss(output, targetS)
# print(cams_loss_result,content_loss_result,style_loss_result)